...
class Tracker {
    ...
    addNewRoleQuery(title, salary, department){
        ...
        // // change department name back to department id
        // const department_id = this.getDepartmentId(department);
        // console.log('did', department_id)
    ...
    getDepartmentId(departmentName){
        console.log('getID dep name', departmentName)
        db.promise().query(`SELECT id FROM departments WHERE name = ?`, departmentName)
            .then(([rows,fields]) => {
                console.log(rows[0].id);
                return rows[0].id
            }).then(departmentId => {
